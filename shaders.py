...
class PhongShader(BaseShaderProgram):
    ...
    def __init__(self, name='phong'):
        ...
        # Storing uniforms in a dictionary for easy extension
        self.uniforms = {
            'PVM': Uniform('PVM'),   # project view model matrix
            ...
            'num_lights': Uniform('num_lights')
        }

        # Adding light uniforms dynamically based on MAX_LIGHTS
        for i in range(self.MAX_LIGHTS):
            self.uniforms[f'light_positions[{i}]'] = Uniform(f'light_positions[{i}]')
            self.uniforms[f'Ia[{i}]'] = Uniform(f'Ia[{i}]')
            self.uniforms[f'Id[{i}]'] = Uniform(f'Id[{i}]')
            self.uniforms[f'Is[{i}]'] = Uniform(f'Is[{i}]')
    ...
    def bind_light_uniforms(self, lights, V):
        '''
        Bind multiple lights to the shader.
        :param lights: list of light objects
        :param V: view matrix
        '''
        num_lights = min(len(lights), self.MAX_LIGHTS)
        self.uniforms['num_lights'].bind(num_lights)

        for i in range(num_lights):
            light = lights[i]
            light_pos_view_space = unhomog(np.dot(V, homog(light.position)))

            # Bind each light's position and properties
            self.uniforms[f'light_positions[{i}]'].bind_vector(light_pos_view_space)
            self.uniforms[f'Ia[{i}]'].bind_vector(np.array(light.Ia, 'f'))
            self.uniforms[f'Id[{i}]'].bind_vector(np.array(light.Id, 'f'))
            self.uniforms[f'Is[{i}]'].bind_vector(np.array(light.Is, 'f'))
