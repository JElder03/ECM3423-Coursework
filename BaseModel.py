...
class BaseModel:
    ...
    def draw(self, Mp=poseMatrix()):
        ...
        if self.visible:
            ...
            is_reflection = np.linalg.det(np.matmul(Mp, self.M)) < 0
            faces = self.mesh.faces

            # Adjust the winding order only for triangular faces if reflection is applied
            if is_reflection and faces is not None:
                if faces.shape[1] == 3:  # Only proceed if we have triangular faces
                    # Reverse only the last two vertices in each face to adjust the winding order
                    faces = np.array([[face[0], face[2], face[1]] for face in faces], dtype=np.uint32)

            # Draw elements based on whether we have faces or not
